...
@Component({
  ...
  providers: [ViajesService, { provide: DateAdapter, useClass: AppDateAdapter }, { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS }, { provide: MAT_DATE_LOCALE, useValue: 'ars-ARS' }]
})
export class ViajesComponent implements OnInit {
  ...
  public guardarViaje(){
    ...
    //fin modulo
    const identity = JSON.parse(localStorage.getItem('identity'));
    this._ViajesService.editar(this.Viajes, identity.token, this.Viajes.id).subscribe(
      response => {
        console.log(response);
        if (response.estado != "ERROR") {
          this.completecampo = null;
  ...
  onValorViajeIgualForm() {
    if (this.ValorViaje.importe != '') {
      this._ViajesService.viajeAutValor(this.ValorViaje, this.identity.token, this.idValorViaje).subscribe(
        response => {
          this.showNotification('top', 'center', response.mensaje, 'warning');
  ...
  onCostoIgualForm() {
    if (this.ValorViaje.importe != '') {
      this._ViajesService.viajeAutCosto(this.ValorViaje, this.identity.token, this.idCosto).subscribe(
        response => {
          this.showNotification('top', 'center', response.mensaje, 'warning');
  ...
  onObservacion() {
    ...
    this._ViajesService.estadoAuth(this.identity.token, this.cancelviajeestado, this.Observaciones
    ).subscribe(
      response => {
        console.log(response);
        this.showNotification('top', 'center', response.mensaje, 'success');
