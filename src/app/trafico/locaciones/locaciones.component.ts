...
export class LocacionesComponent implements OnInit {
  ...
  constructor(private _LocacionesService: LocacionesService, private mapsAPILoader: MapsAPILoader, private ngZone: NgZone) {
  ...
  onSubmit() {
    ...
    this.locaciones.direccion = this.direccion;
    this.locaciones.latitud = $("#latitud").val();
    this.locaciones.longitud = $("#longitud").val();
  ...
  public All() {
    this._LocacionesService.getAll(this.identity.token).subscribe(
      response => {
        if (response.estado != "ERROR") {
          this.dataTable = {
            headerRow: ['locacion', 'direccion', 'estado', 'Borrar'],
            footerRow: ['locacion', 'direccion', 'estado', 'Borrar'],
  ...
  onEdit() {
    ...
    if (this.direccion != '') {
      this.locaciones.direccion = this.direccion;
      this.locaciones.latitud = $("#editlatitud").val();
      this.locaciones.longitud = $("#editlongitud").val();
    }
